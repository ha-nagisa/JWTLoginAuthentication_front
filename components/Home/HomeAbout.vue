<template>
  <div>
    <v-card-title class="pb-8 text-subtitle-2 justify-center"> このアプリケーションの作り方は、下記URLに公開されています。 </v-card-title>
    <v-card-title class="text-subtitle-2 justify-center"> 「Rails6とNuxt.jsで作るユーザーJWT認証付きシングルページアプリケーション」 </v-card-title>
    <v-card-text class="text-center">
      <a :href="blogUrl" rel="nofollow" target="_blank" class="text-decoration-none">
        {{ blogUrl }}
      </a>
    </v-card-text>
    <v-card-title class="text-subtitle-2 justify-center"> 採用している技術 </v-card-title>
    <v-container fluid :style="{ maxWidth: '960px' }">
      <v-row justify="space-around">
        <div v-for="(tec, i) in technologies" :key="`tec-${i}`" class="text-center pa-2">
