<template>
  <div>
    <v-card-title class="font-weight-bold justify-center"> メンバー </v-card-title>
    <v-row justify="space-around">
      <v-col v-for="(member, i) in members" :key="`member-${i}`">
        <v-list-item>
          <v-list-item-icon>
            <img :src="member.img" :alt="member.nickname" width="50" height="50" />
          </v-list-item-icon>
          <v-list-item-content>
            <div>
              {{ member.name }}
            ...
            <v-list-item-action-text>
              <v-btn v-if="member.twitter" :href="member.twitter" target="_blank" rel="noopener noreferrer" small icon>
                <v-icon size="18"> mdi-twitter </v-icon>
              </v-btn>
              <v-btn v-if="member.slack" :href="member.slack" target="_blank" rel="noopener noreferrer" small icon>
                <v-icon size="18"> mdi-slack </v-icon>
              </v-btn>
            </v-list-item-action-text>
          </v-list-item-content>
        </v-list-item>
    ...
    <v-card-title class="font-weight-bold justify-center"> 会社情報 </v-card-title>
        ...
        <v-list flat dense>
          ...
          <template v-for="(info, i) in infomations">
            <v-list-item :key="`info-list-${i}`">
              ...
              <v-list-item-content>
                <a v-if="info.link" :href="info.link" rel="nofollow" target="_blank" class="text-decoration-none">
                  {{ info.link }}
                </a>
